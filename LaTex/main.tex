...
\begin{definition}
    Definim el conjunt $\mathcal{S}$ de sons simples com: $$\mathcal{S}=\{s=(f,a,\varphi)^\mathrm{T}\in\RR^3:s\text{ és el so d'equació }x(t)=a\sin(2\pi ft+\varphi)\}.$$
\end{definition}
\begin{definition}
    Definim el conjunt $\mathcal{C}$ de sons complexos com: $$\mathcal{C}=\left\{M=
    \begin{pmatrix}
        f_1 & f_2 & \cdots & f_n\\
        a_1 & a_2 & \cdots & a_n\\
        \varphi_1 & \varphi_2 & \cdots & \varphi_n
    \end{pmatrix}\in\mathcal{M}_{3\times n}(\RR):M\text{ és el so d'equació }x(t)=\sum_{i=1}^na_i\sin(2\pi f_it+\varphi_i)\right\}.$$
\end{definition}
A partir d'aquestes dues definicions observem que $\mathcal{S}\subset\mathcal{C}$. L'objectiu ara és definir operacions que d'una banda ens permetin combinar dos (o més) sons complexos en un de sol i d'altra banda ens permetin calibrar la intensitat d'un so.
\begin{definition}
    Siguin $M_1,M_2\in\mathcal{C}$ tals que $$M_1=
    \begin{pmatrix}
        f_1 & f_2 & \cdots & f_n\\
        a_1 & a_2 & \cdots & a_n\\
        \varphi_1 & \varphi_2 & \cdots & \varphi_n
    \end{pmatrix}\quad\text{i}\quad M_2=
    \begin{pmatrix}
        f_{n+1} & f_{n+2} & \cdots & f_m\\
        a_{n+1} & a_{n+2} & \cdots & a_m\\
        \varphi_{n+1} & \varphi_{n+2} & \cdots & \varphi_m
    \end{pmatrix}.$$ Definim la \textit{suma $+$ de $M_1$ i $M_2$} com: $$M_1+M_2:=
    \begin{pmatrix}
        f_1 & f_2 & \cdots & f_n & f_{n+1} & f_{n+2} & \cdots & f_m\\
        a_1 & a_2 & \cdots & a_n & a_{n+1} & a_{n+2} & \cdots & a_m\\
        \varphi_1 & \varphi_2 & \cdots & \varphi_n & \phi_{n+1} & \phi_{n+2} & \cdots & \phi_m
    \end{pmatrix}.$$
\end{definition}
\begin{definition}
    Sigui $\lambda\in\RR$ i $M\in\mathcal{C}$ tal que $$M=
    \begin{pmatrix}
        f_1 & f_2 & \cdots & f_n\\
        a_1 & a_2 & \cdots & a_n\\
        \varphi_1 & \varphi_2 & \cdots & \varphi_n
    \end{pmatrix}.$$ Definim el \textit{producte $\cdot$ de $M$ per l'escalar $\lambda$} com: $$\lambda\cdot M:=
    \begin{pmatrix}
        f_1 & f_2 & \cdots & f_n\\
        \lambda a_1 & \lambda a_2 & \cdots & \lambda a_n\\
        \varphi_1 & \varphi_2 & \cdots & \varphi_n
    \end{pmatrix}$$
\end{definition}
Vegem ara una propietat important que es compleix a $\mathcal{C}$ quan implementem aquestes operacions:
\begin{theorem}
    $(\mathcal{C},+,\cdot)$ és un $\RR$-espai vectorial.
\end{theorem}
\begin{proof}
    Cal comprovar totes els axiomes d'espai vectorial.
    \begin{enumerate}
        \item d
    \end{enumerate}
\end{proof}
