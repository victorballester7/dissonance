...
double disso_complexa(int harmonics_F1,int harmonics_F2,double F1[][2],double F2[][2]);
...
int main(){
	...
	switch(n){
		...
		case 3:{//Valor de dissonacia entre dos sons introduits manualment.
			/*FILE *fitxer_harmonicsF1,*fitxer_harmonicsF2;
			fitxer_harmonicsF1=fopen("harmonicsF1.txt","r",);
			fitxer_harmonicsF2=fopen("harmonicsF2.txt","r",);
			if(fitxer_harmonicsF1==NULL || fitxer_harmonicsF2==NULL){
				printf("No s'han pogut obrir els fitxers.");
				return 1;
			}
			while(EOF!=fscanf(fitxer_harmonics,"%lf %lf %lf %lf",);*/
			printf("Introduir la frequencia fonamental del so 1: ");
			scanf("%lf",&f1);
			printf("Introduir la frequencia fonamental del so 2: ");
			scanf("%lf",&f2);
			printf("Introduir quants harmonics es volen utilitzar: ");
			scanf("%i",&harmonics);
			...
			double F1[harmonics][2],F2[harmonics][2];
			nota_musical_manual(f1,harmonics,F1);
			nota_musical_manual(f2,harmonics,F2);
			printf("\nDissonancia: %.16G\n",disso_complexa(harmonics,harmonics,F1,F2));
			//fclose(fitxer_harmonics);
			break;
		}
		default:
...
void nota_musical_manual(double freq_fonamental,int harmonics,double F[harmonics][2]){
	printf("Introduir l'amplitud de cada harmonic del so amb frequencia fonamental %.16G:\n",freq_fonamental);
	for(int i=1;i<=harmonics;i++){
		F[i-1][0]=i*freq_fonamental;
		printf("Amplitud de l'harmonic %i: ",i);
		scanf("%lf",&F[i-1][1]);
	}
}
...
double disso_simple(double f1,double f2,double A1,double A2){
	double max_diss=0.25,x=fabs(f1-f2)/(CBW((f1+f2)/2)*max_diss);//max_diss=maxima dissonancia en termes de la criticalband.
	return fmin(A1,A2)*x*exp(1-x);
}
double disso_complexa(int harmonics_F1,int harmonics_F2,double F1[][2],double F2[][2]){//llista d'harmonics. A la primera columna la freqüència; a la segona, l'amplitud.
