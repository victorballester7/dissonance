...
\begin{frame}{Objectiu del treball}
    És ben sabut que hi ha combinacions de notes musicals que sonen més bé que d'altres. Des de l'antiguitat se sap que això correspon al fet que certs paràmetres identificatius de les notes siguin proporcionals a números enters senzills.\par\vspace{1cm}
    L'objectiu del treball és donar una modelització matemàtica de com mesurar el grau de dissonància produït quan toquem dos (o més) notes musicals simultàniament.
\end{frame}
...
\begin{frame}{Operacions definides sobre $\mathcal{C}$}
    Suposem $\X,\Y\in\mathcal{C}$ tals que $\X=\{s_1,\ldots,s_n\}$, on $s_i\in\mathcal{S}$ per $i=1,\ldots,n$.\par\pause
    \begin{defin}
        Definim la suma $\oplus$ de $\X$ i $\Y$ com l'aplicació:
...
\begin{frame}{Model per a sons complexos}
    \vspace{-0.1cm}
    \begin{defin}
        Definim la funció $d$ de \textit{dissonància relativa} entre $\X,\Y\in\mathcal{C}$ com:
    $$d(\X,\Y):=\frac{1}{2}\sum_{i=1}^n\sum_{j=1}^m\delta(s_i,r_j)$$
    \end{defin}\pause\vspace{-0.05cm}
    \begin{alertblock}{Mesura de dissonància de sons complexos}
        Definim la dissonància $D$ d'un so complex $\X$ com: $$D(\X):= d(\X,\X)$$
    \end{alertblock}
    Es pot comprovar que: $$D(\X\oplus\Y)=D(\X)+D(\Y)+2d(\X,\Y)$$\pause O més en general, donats $n$ sons complexos $\X_1,\ldots,\X_n\in\mathcal{C}$ es complex que:\par \vspace{-0.2cm}
...
\end{frame}
\begin{frame}
    Si considerem dues notes musicals $\N_1$ i $\N_2$ amb 7 harmònics de la forma: $$\N_i=\left\{(f_i,1),\left(2f_i,\frac{1}{2^\alpha}\right),\ldots,\left(7f_i,\frac{1}{7^\alpha}\right)\right\}\quad\text{ per }i=1,2,$$ suposant $a_k=\frac{1}{k^{0.4}}$ amb $\alpha=0.4$:\pause
    \begin{figure}
        \centering
...
\begin{frame}{Conclusions i refinaments}
    ...
    \begin{itemize}
        ...
        \item Fer un estudi amb diversos tipus d'instruments (de vent, de corda, de percussió...)\pause
